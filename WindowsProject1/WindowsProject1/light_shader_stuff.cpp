...
light_shader_stuff::light_shader_stuff()
{
	vertex_shader = 0;
	pixel_shader = 0;
	layout = 0;
...
bool light_shader_stuff::init(ID3D11Device * device, HWND hwnd, LPCWSTR vs_fn, LPCWSTR ps_fn)
{
	HRESULT check;
	...
	texture_sampler_desc.AddressU = D3D11_TEXTURE_ADDRESS_BORDER;
	texture_sampler_desc.AddressV = D3D11_TEXTURE_ADDRESS_BORDER;
	...
	texture_sampler_desc.AddressW = D3D11_TEXTURE_ADDRESS_BORDER;
...
bool light_shader_stuff::render(ID3D11DeviceContext* device_context, int index_count, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView* texture)
{
	HRESULT check;
...
void GraphicsProj::light_shader_stuff::init_info(XMFLOAT3 light_direction, XMFLOAT4 diffuse_color)
{
...
void light_shader_stuff::clean_up()
{
