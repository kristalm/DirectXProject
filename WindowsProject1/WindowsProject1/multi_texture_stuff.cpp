...
namespace GraphicsProj
{
	...
	multi_texture_stuff::multi_texture_stuff()
	{
		vertex_shader = 0;
		pixel_shader = 0;
		layout = 0;
	...
	bool GraphicsProj::multi_texture_stuff::init(ID3D11Device * device, HWND hwnd, LPCWSTR vs_fn, LPCWSTR ps_fn)
	{
		HRESULT check;
	...
	bool GraphicsProj::multi_texture_stuff::render(ID3D11DeviceContext * device_context, int index_count, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView** shader)
	{
		HRESULT check;
		...
		device_context->PSSetShaderResources(0, 2, shader);
	...
	void GraphicsProj::multi_texture_stuff::clean_up()
	{
