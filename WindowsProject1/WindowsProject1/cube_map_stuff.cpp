...
namespace GraphicsProj
{
	...
	bool GraphicsProj::cube_map_stuff::init(ID3D11Device * device, HWND hwnd, LPCWSTR vs_fn, LPCWSTR ps_fn)
	{
		HRESULT check;
		...
		D3DCompileFromFile(ps_fn, 0, 0, "PShader", "ps_4_0", D3DCOMPILE_DEBUG, 0, &pixel_shader_blob, &error_message);
		if (FAILED(check))
		{
			// If the shader failed to compile it should have writen something to the error message.
			if (error_message)
	...
	bool GraphicsProj::cube_map_stuff::render(ID3D11DeviceContext * device_context, int index_count, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView* shader)
	{
		HRESULT check;
	...
	void GraphicsProj::cube_map_stuff::clean_up()
	{
	...
	void cube_map_stuff::load(ID3D11Device * device, LPCWSTR fn)
	{
		CreateDDSTextureFromFile(device, fn, nullptr, &cube_resource);
	}
	ID3D11ShaderResourceView * cube_map_stuff::get_resource_view()
	{
		return cube_resource;
	}
}