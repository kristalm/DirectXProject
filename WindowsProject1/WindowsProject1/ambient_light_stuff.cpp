...
ambient_light_stuff::ambient_light_stuff()
{
	vertex_shader = 0;
	pixel_shader = 0;
	layout = 0;
...
bool ambient_light_stuff::init(ID3D11Device * device, HWND hwnd, LPCWSTR vs_fn, LPCWSTR ps_fn)
{
	HRESULT check;
...
bool ambient_light_stuff::render(ID3D11DeviceContext* device_context, int index_count, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView* texture)
{
	HRESULT check;
...
void GraphicsProj::ambient_light_stuff::init_info(XMFLOAT4 ambient_color, XMFLOAT3 light_direction, XMFLOAT4 diffuse_color)
{
...
void ambient_light_stuff::clean_up()
{
