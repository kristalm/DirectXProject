...
{
	specular_lighting_stuff::specular_lighting_stuff()
	{
		vertex_shader = 0;
		pixel_shader = 0;
		layout = 0;
	...
	bool GraphicsProj::specular_lighting_stuff::init(ID3D11Device * device, HWND hwnd, LPCWSTR vs_fn, LPCWSTR ps_fn)
	{
		HRESULT check;
		...
		camera_buffer_desc.ByteWidth = sizeof(camera_struct);
		...
		device->CreateBuffer(&camera_buffer_desc, NULL, &camera_buffer);
	...
	bool GraphicsProj::specular_lighting_stuff::render(ID3D11DeviceContext * device_context, int index_count, XMMATRIX world, XMMATRIX view, XMMATRIX proj, ID3D11ShaderResourceView * texture, XMFLOAT3 camera_pos)
	{
		HRESULT check;
		...
		check = device_context->Map(camera_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_subresource2);
		
		camera_struct* data3;
		data3 = (camera_struct*)mapped_subresource2.pData;
		data3->pos = camera_pos;
		data3->padding = 0.0f;

		device_context->Unmap(camera_buffer, 0);
		device_context->VSSetConstantBuffers(1, 1, &camera_buffer);

		//set shader resources
		device_context->PSSetShaderResources(0, 1, &texture);

		//map light pixel buffer
		...
		data2->ambient_color = info->get_ambient_color();
		data2->diffuse_color = info->get_diffuse_color();
		data2->direction = info->get_direction();
		data2->color = info->get_spec_color();
		data2->intensity = info->get_intensity();
	...
	void specular_lighting_stuff::init_info(XMFLOAT4 ambient_color, XMFLOAT3 light_direction, XMFLOAT4 diffuse_color, XMFLOAT4 spec_color, float intensity)
	{
		info = new light_stuff;
		info->set_ambient_color(ambient_color);
		info->set_diffuse_color(diffuse_color);
		info->set_spec_color(spec_color);
		info->set_intensity(intensity);
		info->set_direction(light_direction);
	}

	void GraphicsProj::specular_lighting_stuff::clean_up()
	{
