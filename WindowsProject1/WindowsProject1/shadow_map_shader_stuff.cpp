...
namespace GraphicsProj
{
	...
	shadow_map_shader_stuff::shadow_map_shader_stuff()
	{
		vertex_shader = 0;
		pixel_shader = 0;
		layout = 0;
	...
	bool shadow_map_shader_stuff::init(ID3D11Device * device, HWND hwnd, LPCWSTR vs_fn, LPCWSTR ps_fn)
	{
		...
		device->CreateSamplerState(&texture_sampler_desc, &sample_state_wrap);
		...
		device->CreateSamplerState(&texture_sampler_desc, &sample_state_clamp);
	...
	bool shadow_map_shader_stuff::render(ID3D11DeviceContext * device_context, int index_count, XMMATRIX world, XMMATRIX view, XMMATRIX proj, XMMATRIX light_view, XMMATRIX light_proj, ID3D11ShaderResourceView * texture, ID3D11ShaderResourceView* depth_texture )
	{
		HRESULT check;
		...
		world = XMMatrixTranspose(world);
		light_view = XMMatrixTranspose(light_view);
		light_proj = XMMatrixTranspose(light_proj);
		...
		device_context->PSSetSamplers(0, 1, &sample_state_wrap);
		device_context->PSSetSamplers(1, 1, &sample_state_wrap);

		device_context->DrawIndexed(index_count, 0, 0);

		return true;
	}
	void shadow_map_shader_stuff::init_info(light_stuff* stuff)
	{
		info = new light_stuff;
		info = stuff;
	}
	void shadow_map_shader_stuff::clean_up()
	{
		vertex_shader->Release();
		pixel_shader->Release();
		layout->Release();
		cbuffer->Release();
		sample_state_wrap->Release();
		sample_state_clamp->Release();
